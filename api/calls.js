...
module.exports = async function handler(req, res) {
  ...
  const grantCallTypes = parseGrantCallTypes(query);
...
function parseGrantCallTypes(query) {
  const raw = query.grantCallTypes || query.grantCallType || query.types || query.type || "";
  const values = Array.isArray(raw) ? raw : String(raw).split(",");
  const allowed = new Set(Object.keys(GRANT_CALL_TYPE_MAP));
  const selected = values.map((value) => String(value).trim()).filter((value) => allowed.has(value));
  return selected.length ? Array.from(new Set(selected)) : Object.keys(GRANT_CALL_TYPE_MAP);
}
...
function cacheKey({ page, pageSize, searchText, includeClosed, grantCallTypes }) {
  return JSON.stringify({ page, pageSize, searchText, includeClosed, grantCallTypes });
}
...
async function getLivePage(options) {
  ...
  const page = await fetchPage(options.page, options.pageSize, CONFIG.REQUEST_TIMEOUT_MS, options.searchText, options.grantCallTypes);
  const items = [];
  ...
  const payload = {
    ...
    limits: {
      ...
      grantCallTypes: options.grantCallTypes,
...
function buildSearchQuery(grantCallTypes = Object.keys(GRANT_CALL_TYPE_MAP)) {
  return {
    bool: {
      must: [
        { terms: { type: grantCallTypes } },
...
async function fetchPage(pageNumber, pageSize, timeoutMs, searchText = "", grantCallTypes = Object.keys(GRANT_CALL_TYPE_MAP)) {
  let lastError = null;
  for (let attempt = 1; attempt <= Math.max(1, CONFIG.REQUEST_RETRIES); attempt += 1) {
    try {
      return await fetchPageOnce(pageNumber, pageSize, timeoutMs, searchText, grantCallTypes);
    } catch (error) {
      lastError = error;
    }
...
async function fetchPageOnce(pageNumber, pageSize, timeoutMs, searchText = "", grantCallTypes = Object.keys(GRANT_CALL_TYPE_MAP)) {
  ...
  body.append("query", new Blob([JSON.stringify(buildSearchQuery(grantCallTypes))], { type: "application/json" }));
  body.append("languages", new Blob([JSON.stringify(["en"])], { type: "application/json" }));
  body.append("displayFields", new Blob([JSON.stringify([
    "type",
    "identifier",
    "reference",
    "title",
...
function normalizeItem(item) {
  ...
  return {
    ...
    [GRANT_CALL_TYPE_COLUMN]: nonEmptyOrNA(mapGrantCallType(actionRaw)),
...
function mapGrantCallType(raw) {
  const value = String(raw || "").trim();
  if (GRANT_CALL_TYPE_MAP[value]) return GRANT_CALL_TYPE_MAP[value];
  return value || "N/A";
}
